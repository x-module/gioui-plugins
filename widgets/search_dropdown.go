...
type SearchDropDown struct {
	...
	onSelectedChange func(value SearchDropDownOption)
...
func (c *SearchDropDown) SetOnChanged(f func(value SearchDropDownOption)) {
...
func NewSearchDropDownWithoutBorder(th *theme.Theme) *SearchDropDown {
	c := &SearchDropDown{
		SearchDropDown: &widget.Bool{Value: true},
		menuContextArea: component.ContextArea{
			Activation:       pointer.ButtonPrimary,
...
func (c *SearchDropDown) updateMenuItems(text string) {
	if text == "" {
		c.menu.Options = c.menu.Options[:0]
		for _, opt := range c.options {
			current := opt
			c.menu.Options = append(c.menu.Options, func(gtx layout.Context) layout.Dimensions {
				itm := component.MenuItem(c.theme.Material(), &current.clickable, current.Text+"["+current.Value+"]")
		...
		c.menu.Options = c.menu.Options[:0]
		for _, opt := range c.options {
			if strings.Contains(opt.Text, text) || strings.Contains(opt.Value, text) {
				current := opt
				c.menu.Options = append(c.menu.Options, func(gtx layout.Context) layout.Dimensions {
					itm := component.MenuItem(c.theme.Material(), &current.clickable, current.Text+"["+current.Value+"]")
					itm.HoverColor = c.theme.Color.DropDownItemHoveredGrayColor
